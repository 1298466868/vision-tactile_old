...
class TimeSFormer(nn.Module):
    """ Vision Transformere
    """
    def __init__(self, Tactile_img_size, Visual_img_size, Tactile_patch_size, Visual_patch_size, in_chans=3, num_classes=1000, embed_dim=768, depth=12,
        ...
        self.patch_embed_visual = PatchEmbed(
        ...
        trunc_normal_(self.pos_embed_visual, std=.02)
        trunc_normal_(self.cls_token_visual, std=.02)
        ...
        if self.attention_type == 'divided_space_time':
            i = 0
            for m in self.blocks_visual.modules():
                m_str = str(m)
                if 'Block' in m_str:
                    if i > 0:
    ...
    def forward_features_visual(self, x):
        #print(x.shape)
        B = x.shape[0]
        x, T, W = self.patch_embed_visual(x)
        cls_tokens = self.cls_token_visual.expand(x.size(0), -1, -1)
        x = torch.cat((cls_tokens, x), dim=1)

        ## resizing the positional embeddings in case they don't match the input at inference
        if x.size(1) != self.pos_embed_visual.size(1):
            pos_embed = self.pos_embed_visual
        ...
        if self.attention_type != 'space_only':
            ...
            if T != self.time_embed_visual.size(1):
                time_embed = self.time_embed_visual.transpose(1, 2)
                new_time_embed = F.interpolate(time_embed, size=(T), mode='nearest')
                new_time_embed = new_time_embed.transpose(1, 2)
                x = x + new_time_embed
            else:
                x = x + self.time_embed_visual
            x = self.time_drop(x)
            x = rearrange(x, '(b n) t m -> b (n t) m',b=B,t=T)
            x = torch.cat((cls_tokens, x), dim=1)

        ## Attention blocks
        for blk in self.blocks_visual:
            x = blk(x, B, T, W)
...
if __name__ == '__main__':
    ...
    ts = TimeSFormer(img_size=(120,160), patch_size=(12,16), in_chans=3, num_classes=10, embed_dim=128, depth=1, num_heads=4, mlp_ratio=4, qkv_bias=False, qk_scale=None, drop_rate=0, attn_drop_rate=0., drop_path_rate=0., hybrid_backbone=None, norm_layer=nn.LayerNorm, num_frames=8, attention_type='divided_space_time', dropout=0.)
    if(use_gpu):
        ts = ts.to('cuda')
    out = ts(x,y)
    print(out)
